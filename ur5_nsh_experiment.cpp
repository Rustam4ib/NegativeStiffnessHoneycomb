...
//double thorlab_table_height = 0.07; // ur5 is on thorlab (height = 6cm)
...
void PoseShow::pose_now()
{
  ...
  */
	...
	//Z_NOW = nowtransform.getOrigin().getZ() + thorlab_table_height - blackprintedpart_height; // get Z position of tool0 relative to not_thorlab_table
...

{
  ...
  ros::Subscriber sub_force_z = nh.subscribe("/weiss_wrench", 100, force_z_Callback);
	...
	URutil Experiment;
  	//Z_TARGET = 0.30-0.16; // Target position for Z-coordinate of end-effector
	Z_TARGET = weiss_height - honeycomb_buckling_height; // Target position for Z-coordinate of end-effector
	//Z_TARGET = weiss_height-0.01; // Target position for Z-coordinate of end-effector
  ...
  while((ros::ok()) && (ros::Time::now() - start_time < timeout)) {
		...
		if( (ros::ok()) && ((Z_NOW)<(Z_TARGET + 0.02)) && ((Z_NOW)>(Z_TARGET)) ){
		ros::Subscriber sub_force_z = nh.subscribe("/weiss_wrench", 100, force_z_Callback);
